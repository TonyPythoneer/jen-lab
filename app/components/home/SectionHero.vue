...
const EXIT_DISTANCE = 130; // % of own width each portrait travels before it's gone
...
const knowsStyle = computed(() => ({
  transform: `translateX(${-progress.value * EXIT_DISTANCE}%)`,
...
<template>
  <section ref="pinWrapper" class="relative h-[200vh]">
    <div
      class="sticky top-[var(--site-header-h)] h-[calc(100vh_-_var(--site-header-h))] bg-ash-white rounded-card overflow-hidden flex flex-col"
    >
      ...
      <div
        class="relative z-10 px-6 md:px-12 pt-10 md:pt-14 pb-16 md:pb-20 max-w-[1100px] w-full mx-auto text-center space-y-8 flex flex-1 flex-col justify-center items-center"
      >
        <h1
          ref="title"
