...
await p.evaluate(() => { window.__foodMapDebug.map.setZoom(16, { animate: true }); });
await p.waitForTimeout(1400);
await p.screenshot({ path: ".planning/debug/final-zoom16.png" });

// pan via drag
await p.mouse.move(550, 400); await p.mouse.down();
for (let i=0;i<20;i++){ await p.mouse.move(550 - i*8, 400 - i*4); await p.waitForTimeout(12); }
