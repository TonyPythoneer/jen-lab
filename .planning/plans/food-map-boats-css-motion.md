...
- Support `prefers-reduced-motion` *at ease* — it already works (static placement) and will be checked at build time, not runtime.
...
- **CSS:** Speed is encoded as `duration`. A vessel with 2× speed gets 0.5× duration. Works, but the mapping from m/s → duration is a heuristic; very slow/fast vessels may not feel *exactly* like the original.
