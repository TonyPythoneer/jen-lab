...
**Date:** 2026-06-02
**Branch:** `chore/simplify-food-map`
...
## Implementation outcome (Tier 1)

Shipped and verified in-browser (computed-style diffs + screenshots on `:3500`):

- **Token bridge** — `food-map.css` now maps Nuxt UI's `--ui-*` design tokens to the food-map
  parchment tokens inside `.food-map-page`. So Nuxt UI components adopt the atlas palette with no
  per-component colour overrides. (Confirmed: a probe `<div class="text-default bg-default">` resolves
  to foreground/paper.) The chrome tokens are **static across all four map themes** — only the map
  tiles recolour — so the bridge needs no theme wiring.
- **Search field → `UInput`** (`FoodMapListDrawer.vue`) — removed ~26 lines of `.search-field*` CSS +
  inline SVG. Computed result is pixel-identical (paper bg, `--border-strong` ring, 5px radius, 14px
  serif, `--muted-foreground` icon, 38px tall).
- **Rec chips → `UBadge`** (`FoodMapDetail.vue`) — removed the `.rec-chip` rules. Pixel-identical
  (paper bg, 1px `--foreground` inset ring, full radius, 12px, 6px×10px padding, `✦` accent star).

**Not shipped — `UButton` for the icon buttons (home, map-controls, theme trigger).** Blocked by a real
constraint: `food-map.css` carries an intentional `:where(.food-map-page) button/a { color/background/
padding reset }`. That reset is **unlayered**, and in Tailwind v4 unlayered CSS beats `@layer utilities`,
so `UButton`'s utility classes lose to the reset (`<button>` collapses to 18px, transparent bg; `<a>`
keeps the wrong inherited colour). The only fixes are `!important` on every button utility, or
re-layering a load-bearing global reset — both risk side effects. **Kept as bespoke CSS.** `UInput` /
`UBadge` are unaffected because they render `<input>` / `<span>`, which the reset does not target.

**Also kept (not genuine wins):** drawer tabs (`UTabs` is content-oriented; here the tabs drive an
external `v-if`, not tab panels), the map-controls zoom group (a bespoke segmented control with a shared
container + internal divider — `UButtonGroup` doesn't map cleanly), the theme-menu divider (`USeparator`
is net-neutral), and inline SVG → `UIcon` (zero line savings).

**Net:** ~41 lines of custom CSS removed; two components now use Nuxt UI; one reusable token bridge added.
The honest finding from real code: this page is a bespoke cartographic system, so heavy Nuxt UI adoption
is not a net simplification — most swaps trade CSS for an equal pile of token overrides plus coupling.
...
food-map-scoped variant. Without that, these are _not_ zero-side-effect changes. The per-unit analysis
...
| Restaurant row active state → `UButton`/`UCard` | The `inset 3px 0 0 var(--accent)` left border is the _only_ on-list selection signal; `grid-template-columns: 24px 1fr auto` must not shift; emoji `--ink-filter-soft` must stay.                                      | Keep custom unless a true 1:1 wrapper is proven.                                                                                 |

---

## Tier 3 — Keep as-is (do **not** touch; this is the load-bearing core)

- **Pin pop-in animation** (`@keyframes r-pin-pop`, css ~936–952): scale `0.7→2.26→1.8→2.12→2`. This _is_ the selection UX.
...
7. _If wired:_ `UInput` (search field)
...
_Generated by the `food-map-simplify-analysis` workflow (6 analysis agents + 1 synthesis), 2026-06-02.
Full raw findings retained in the workflow transcript._
