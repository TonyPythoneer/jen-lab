// Bakes the terrain for /sydney-food-map-3d into public/food-map-3d/: DEM (AWS
// Terrarium, SRTM/USGS) + satellite imagery (EOX Sentinel-2 cloudless, CC BY 4.0)
// at z14 over the restaurant bbox. Landmarks are real 3D models in models/.
...
console.log(
  `base z=${Z} tiles x:${x0}..${x1} y:${y0}..${y1} → ${(x1 - x0 + 1) * (y1 - y0 + 1) * 2} tiles`,
);
  ...
  JSON.stringify(
    {
      ...
      maxLng,
      attribution:
        "Terrain: Terrarium DEM (SRTM, USGS). Imagery: Sentinel-2 cloudless 2023, EOX — CC BY 4.0.",
    },
    null,
    2,
  ),
