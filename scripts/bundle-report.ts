...
function printReport(report: Report) {
  ...
  console.log(
    `Total size: ${fmt(summary.total_parsed_size)}  (gzip: ${fmt(summary.total_gzip_size)})`,
  );
  ...
  for (const [cat, entries] of groups) {
    ...
    for (const c of entries) {
      console.log(
        `│  ${c.id.padEnd(COL)} ${fmt(c.parsed_size).padStart(9)} ${fmt(c.gzip_size).padStart(9)}`,
      );
      if (c.top_packages.length) {
        for (const p of c.top_packages) {
          console.log(
            `│    ↳ ${p.name.padEnd(COL - 4)} ${fmt(p.parsed_size).padStart(9)} ${fmt(p.gzip_size).padStart(9)}`,
          );
        }
      }
    }
...
function compareReports(
  current: Report,
  baseline: Report,
): { summary: Record<string, number>; chunks: MergedChunk[] } {
...
function printCompare(compare: ReturnType<typeof compareReports>) {
  ...
  console.log(
    `Total: ${sign(summary.total_parsed_diff)} size  (gzip: ${sign(summary.total_gzip_diff)})`,
  );
  ...
  for (const [cat, entries] of groups) {
    ...
    for (const c of entries) {
      const curr = c.current
        ? `${fmt(c.current.parsed_size).padStart(9)} ${fmt(c.current.gzip_size).padStart(9)}`
        : "        —         —";
      const base = c.baseline
        ? `${fmt(c.baseline.parsed_size).padStart(9)} ${fmt(c.baseline.gzip_size).padStart(9)}`
        : "        —         —";
      const baseGzip = c.baseline?.gzip_size ?? 0;
      const diffStr =
        c.status === "unchanged" ? "—" : `${sign(c.gzip_diff)} (${pct(c.gzip_diff, baseGzip)})`;
      const statusTag =
        c.status === "added"
          ? "[+]"
          : c.status === "removed"
      ...
      if (c.current?.top_packages.length) {
        for (const p of c.current.top_packages) {
          console.log(
            `│    ↳ ${p.name.padEnd(COL - 2)} ${fmt(p.parsed_size).padStart(9)} ${fmt(p.gzip_size).padStart(9)}`,
          );
        }
      }
    }
