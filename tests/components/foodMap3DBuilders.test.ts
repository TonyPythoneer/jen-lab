...
import { LANDMARKS } from "~/components/food-map/landmarks";
...
describe("LANDMARKS", () => {
  it("registers Opera House and Harbour Bridge with builders", () => {
    expect(LANDMARKS.map((l) => l.id)).toEqual(["opera-house", "harbour-bridge"]);
  });
  it("each landmark builds an Object3D with children", () => {
    for (const l of LANDMARKS) {
      const obj = l.build(palette);
